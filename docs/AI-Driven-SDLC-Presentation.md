...
## Stage 2: Infrastructure & Database + AI Tools Setup

**What happened:**

- Supabase project provisioned via MCP integration
- App scaffold created (Next.js 16, folder structure, shadcn/ui)
- AI tooling stack configured: Skills, MCP servers, Agents, Commands, Hooks
...
**AI Tools Setup (ChatGPT → Claude verified):**

| Category | Tools | Setup Source |
| -------- | ----- | ------------ |
| Skills   | `doctor-note-domain-skill`, `supabase-skill`, `backend-skill`, `react-best-practices`, `security-review-skill`, `qa-testing-skill`, `ai-sdlc-skill`, `devops-deploy-skill` | ChatGPT drafted, Claude verified |
| MCP      | Supabase MCP, Context7 MCP, Chrome DevTools MCP | ChatGPT drafted, Claude verified |
| Agents   | `architect`, `developer`, `qa`, `reviewer`, `release-manager`, `product-manager` | ChatGPT drafted, Claude verified |
| Commands | `/next-task`, `/create-pr`, `/qa`, `/security-review`, `/review`, `/release-check` | ChatGPT drafted, Claude verified |
| Hooks    | `secret-scan.sh`, `lint.sh`, `typecheck.sh`, `progress-reminder.sh` | ChatGPT drafted, Claude verified |

**Human Verify Gate:**

- Migration files reviewed before applying (`supabase/migrations/`)
- RLS policies verified via `get_advisors()` MCP tool
- AI tool configurations reviewed before committing to `.claude/`
...
- Initial AI tool setup required iteration — ChatGPT drafts needed Claude refinement
