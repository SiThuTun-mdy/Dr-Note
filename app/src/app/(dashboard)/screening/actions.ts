...
export interface ScreeningQueueStats {
...
export async function getScreeningQueueStats(): Promise<ScreeningQueueStats> {
  ...
  const defaultStats: ScreeningQueueStats = {
    awaitingScreening: 0,
    inProgress: 0,
    completedToday: 0,
    queue: [],
  ...
  // Fetch visits waiting for screening
  ...
  // Fetch visits currently in screening
  ...
  const queue = (waitingVisits || []).map((v) => ({
    id: v.id,
