...
export function VisitSummary({ data }: VisitSummaryProps) {
  ...
  // Hydration-safe: defer client-only timestamp to avoid SSR/client mismatch
  useEffect(() => {
    setShowTimestamp(true); // eslint-disable-line react-hooks/set-state-in-effect
  }, []);
    ...
    <>
      ...
      <div className="visit-summary-content space-y-6">
        ...
        <div className="print-only mt-8 pt-4 border-t border-gray-200 text-xs text-gray-500 text-center">
          Generated by Dr.Note {showTimestamp && <>&mdash; {formatDateTime(new Date().toISOString())}</>}
        </div>
      </div>
    </>
  );
