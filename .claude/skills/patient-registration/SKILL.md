...
- issue #20 on git repository for acceptance criteria and constraints
...
3. **Core Registry Fields:**
   ...
   - Email (email format check via regex validation)
   - Mobile Phone Number (10-digit format via regex validation)
   - Date of Birth (Date picker with age validation)
   - Gender [ Male, Femal ] (Accessible selection dropdown)
   - Religion [ Christian, Muslim, Hindu, Buddhist, Other ] (Accessible selection dropdown) Optional field, but if filled, must be one of the listed values.
   - Ethnicity [ African, Asian, Caucasian, Hispanic, Other ] (Accessible selection dropdown) Optional field, but if filled, must be one of the listed values.
   - Address (Text area with character limit) Optional field, but if filled, must not exceed 250 characters.
